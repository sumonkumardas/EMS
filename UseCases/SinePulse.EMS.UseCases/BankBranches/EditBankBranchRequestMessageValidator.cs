...
{
  public class EditBankBranchRequestMessageValidator : AbstractValidator<EditBankBranchRequestMessage>
  {
    private readonly IUniqueBankInfoPropertyChecker _uniqueBankInfoPropertyChecker;
    public EditBankBranchRequestMessageValidator(IUniqueBankInfoPropertyChecker uniqueBankInfoPropertyChecker)
    {
      ...
      RuleFor(x => x.BranchName).Must((m, x) => IsBranchNameExists(m.BranchName, m.BankBranchId))
    ...
    private bool IsBranchNameExists(string branchName, long branchId)
    {
      return _uniqueBankInfoPropertyChecker.IsUniqueBankBranchName(branchName, branchId);
    }
  }
}