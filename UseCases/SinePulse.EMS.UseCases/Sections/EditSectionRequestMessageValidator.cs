...
{
  public class EditSectionRequestMessageValidator : AbstractValidator<EditSectionRequestMessage>
  {
    //private readonly IUniqueSectionPropertyChecker _uniqueSectionPropertyChecker;

    public EditSectionRequestMessageValidator()//(IUniqueSectionPropertyChecker uniqueSectionPropertyChecker)
    {
      ...
      RuleFor(x => x.SectionName).MinimumLength(3).WithMessage("'Section Name' is too short, it should have minimum 3 characters.");
      ...
      RuleFor(x => x.Group).NotEmpty().WithMessage("Please specify Group.");
      RuleFor(x => x.Group).NotNull().WithMessage("Please specify Group.");
