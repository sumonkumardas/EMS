...
{
  public class AddClassTestRequestMessageValidator : AbstractValidator<AddClassTestRequestMessage>
  {
    ...
    public AddClassTestRequestMessageValidator(IUniqueClassTestPropertyChecker uniqueClassTestPropertyChecker,
      IClassTestOverlappingChecker classTestOverlappingChecker)
    {
      ...
      RuleFor(x => x.TestName).Must((m, x) => IsUniqueClassTestName(x, m.SectionId, m.ExamConfigurationId))
    ...
    private bool IsUniqueClassTestName(string classTestName, long sectionId, long examConfigurationId)
    {
      return _uniqueClassTestPropertyChecker.IsUniqueClassTestName(classTestName, sectionId, examConfigurationId);
    }

    private bool IsNonOverlappingClassTestPeriod(AddClassTestRequestMessage test)
    {
      return _classTestOverlappingChecker.IsNonOverlappingClassTestPeriod(
        test.Date + test.StartTime, test.Date + test.EndTime, test.SectionId);
    }
